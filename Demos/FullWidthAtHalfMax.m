...
arguments
        DemoType char {mustBeMember(DemoType, {'Artery', 'Vein'})} = 'Artery';
    end
    ...
    %% Synthetic Noisy Data
        ...
        switch DemoType
            case 'Artery'
                ...
                VesselWidth = LineThickness;

            case 'Vein'
                % Vertical Line with Pronounced Edges
                LineThickness = randi(size(Demo, 1)/4, [1,2]);
                YPosition.Center = [randi(size(Demo, 1)/4, 1), size(Demo, 1)/2 + randi(size(Demo, 1)/4, 1)];
                YPosition.Top = [ceil(YPosition.Center(1) - LineThickness(1)/2), ceil(YPosition.Center(2) - LineThickness(1)/2)];
                YPosition.Bottom = [ceil(YPosition.Center(1) + LineThickness(1)/2), ceil(YPosition.Center(2) + LineThickness(1)/2)];

                MinGapIntensity = 0.25;
                MaxGapIntensity = 1;
                GapIntensity = (MaxGapIntensity - MinGapIntensity)*rand();

                Demo(YPosition.Top(1):YPosition.Bottom(1), :) = MaxGapIntensity;
                Demo(YPosition.Top(2):YPosition.Bottom(2), :) = MaxGapIntensity;
                Demo(YPosition.Bottom(1):YPosition.Top(2), :) = GapIntensity;
                
                VesselWidth = YPosition.Bottom(2) - YPosition.Top(1);
        end
    ...
    %% Create Figure
        ...
        title(Tile1, sprintf('Demo image - Draw ROI\n(%g Pixel Width)', VesselWidth));      % Give a title to tile that contains true line thickness
    ...
    %% Plot Pixel Intensity along ROI Profile
        function UpdateProfile()
            ...
            switch DemoType
                ...
                case 'Vein'
                    [FWHM, Edge1, Edge2, ~, ~] = CalculateFWHMEdge(ROILength, ROIProfile);
            end
            
            set(ROIProfilePlot, 'XData', ROILength, 'YData', ROIProfile);
            set(IntensityMidLine, 'Label', sprintf('Total FWHM ≈ %.1f px', FWHM));
            set(IntensityMidLine, 'Value', 0.5*(max(ROIProfile)+min(ROIProfile)));
            set(Edge1Line, 'Value', Edge1);
            set(Edge2Line, 'Value', Edge2);

            title(Tile1, sprintf('Demo image - Draw ROI\n(%g Pixel Width)', VesselWidth));
            drawnow
        end
end
...
function [FWHM, Edge1, Edge2] = CalculateFWHM(ROIProfilePosition, Intensity)

    MinIntensity = min(Intensity);
    MaxIntensity = max(Intensity);

    scale = 1 / (MaxIntensity - MinIntensity);
    Intensity = (Intensity - MinIntensity) * scale;

    Index = find(Intensity >= 0.5);
    if isempty(Index)
        FWHM = NaN;
        Edge1 = NaN;
        Edge2 = NaN;
    ...
    Edge1 = ROIProfilePosition(Index(1));
    Edge2 = ROIProfilePosition(Index(end));
    FWHM = Edge2 - Edge1;
end

function [TotalFWHM, Edge1, Edge2, EdgeFWHM_Left, EdgeFWHM_Right] = CalculateFWHMEdge(ROIProfilePosition, Intensity)
    ...
    Edge1 = Edge1_Left;   % left-most edge
    Edge2 = Edge2_Right;  % right-most edge

    TotalFWHM = Edge2 - Edge1;
end
